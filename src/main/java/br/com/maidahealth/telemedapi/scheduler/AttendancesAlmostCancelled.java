...
@Component
public class AttendancesAlmostCancelled implements Job {
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		List<Attendance> attendances = attendanceService.getAttendancesAlmostCancelled();
		
		for (Attendance attendance : attendances) {
			String message = attendance.getProfessional().getName() + " continua lhe aguardando.";
			User user = attendance.getInsured().getUser() == null ? attendance.getInsured().getHolder().getUser() : attendance.getInsured().getUser();
			TelemedNotification not = notService.save(message, attendance.getId(), user, TypeActionEnum.ATTENDANCE_ALMOST_CANCELLED);
