...
import {ForgotPasswordResendToken, SignUpValidateToken} from 'domain/usecases';
...
type Props = {
	resendToken: ForgotPasswordResendToken;
	validateToken: SignUpValidateToken;
};

export const ForgotPasswordTokenPage: React.FC<Props> = ({resendToken, validateToken}: Props) => {
	...
	const resendTk = async () => {
		...
		try {
			await resendToken.forgotPasswordResendToken({
				cdPessoa: state.cdPessoa,
			});
		} catch (error) {
			alert(error.message.error.invalidParams.reason);
		}
		setCounter(20);
	};
